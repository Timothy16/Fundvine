...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (!stateId) {
      throw createError({
        statusCode: 422,
        statusMessage: 'State ID is required'
      })
    }
    ...
    const response = await $fetch(`${config.apiBaseUrl}/api/client/v2/location/get-lgas?stateId=${stateId}`, {
      method: 'GET',
      headers: {
    ...
    if (error.response) {
      ...
      throw createError({
        ...
        statusMessage: error.response._data?.message || 'Failed to fetch LGAs',
